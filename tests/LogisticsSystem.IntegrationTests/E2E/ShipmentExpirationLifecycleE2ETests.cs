...
{
    public class ShipmentExpirationLifecycleE2ETests : IClassFixture<CustomWebApplicationFactory>
    {
        private readonly CustomWebApplicationFactory _factory;

        public ShipmentExpirationLifecycleE2ETests(CustomWebApplicationFactory factory)
        {
            _factory = factory;
        }

        [Fact]
        public async Task ShipmentLifecycle_AssignmentExpirationWithAutoReassignment_CompletesSuccessfully()
        {
            ...
            var (custUser, _) = await TestAuthHelper.SeedCustomerAsync(_factory.Services, email: $"cust_exp_{Guid.NewGuid():N}@test.com");
            ...
            var (dispUser, _) = await TestAuthHelper.SeedCustomerAsync(_factory.Services, email: $"disp_exp_{Guid.NewGuid():N}@test.com");
            ...
            var (drv1User, driver1) = await TestAuthHelper.SeedDriverAsync(
                _factory.Services,
                email: $"driver1_exp_{Guid.NewGuid():N}@test.com",
                latitude: 30.0444,
                longitude: 31.2357,
            ...
            var (drv2User, driver2) = await TestAuthHelper.SeedDriverAsync(
                _factory.Services,
                email: $"driver2_exp_{Guid.NewGuid():N}@test.com",
                latitude: 30.0450,
                longitude: 31.2360,
                status: DriverStatus.Available);
            ...
            var createCommand = new CreateShipmentCommand(new CreateShipmentDto
            {
                ...
                DeliveryLatitude = 30.0380,
                DeliveryLongitude = 31.2110,
                Weight = 12m,
                DistanceKm = 6m,
                ShippingCost = 90m,
            ...
            // 4. Age the assignment beyond the expiration threshold (simulate time passing without sleep)
            Guid assignment1Id;
            using (var scope = _factory.Services.CreateScope())
            {
                ...
                // Age timestamp 10 minutes into the past (threshold is 5 min)
                a1.SentAt = DateTime.UtcNow.AddMinutes(-10);
                await db.SaveChangesAsync();
            }

            // 5. Trigger Hangfire Background Expiration Job
            using (var scope = _factory.Services.CreateScope())
            {
            ...
            // 6. Verify Assignment 1 is Expired and Assignment 2 is auto-created for Driver 2
            Guid assignment2Id;
            using (var scope = _factory.Services.CreateScope())
            {
                var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
                ...
                a1.Should().NotBeNull();
                a1!.Status.Should().Be(AssignmentStatus.Expired);
                ...
                a2.Should().NotBeNull("expiration service should automatically assign to the next best available driver");
                ...
                var dbDriver1 = await db.Drivers.FindAsync(driver1.Id);
                dbDriver1.Should().NotBeNull();
                dbDriver1!.Status.Should().Be(DriverStatus.Available, "driver 1 must be available after assignment expired");
            }

            // 7. Driver 2 accepts the auto-reassigned offer
            ...
            // 8. Driver 2 completes pickup, transit, and delivery
            ...
            a1Dto!.Status.Should().Be(AssignmentStatus.Expired);
        ...
        [Fact]
        public async Task ShipmentLifecycle_AssignmentExpirationWithNoAlternativeDriver_AllowsSubsequentManualDispatch()
        {
            ...
            // 1. Seed Customer, Dispatcher, and only Driver 1
            var (custUser, _) = await TestAuthHelper.SeedCustomerAsync(_factory.Services, email: $"cust_exp_man_{Guid.NewGuid():N}@test.com");
            var custToken = await TestAuthHelper.GenerateJwtTokenAsync(_factory.Services, custUser.Id, custUser.Email!, custUser.UserName!, Roles.Customer);
            var customerClient = TestAuthHelper.CreateAuthenticatedClient(_factory, custToken);

            var (dispUser, _) = await TestAuthHelper.SeedCustomerAsync(_factory.Services, email: $"disp_exp_man_{Guid.NewGuid():N}@test.com");
            var dispToken = await TestAuthHelper.GenerateJwtTokenAsync(_factory.Services, dispUser.Id, dispUser.Email!, dispUser.UserName!, Roles.Dispatcher);
            var dispatcherClient = TestAuthHelper.CreateAuthenticatedClient(_factory, dispToken);

            var (drv1User, driver1) = await TestAuthHelper.SeedDriverAsync(
                _factory.Services,
                email: $"driver1_exp_man_{Guid.NewGuid():N}@test.com",
                status: DriverStatus.Available);
            ...
            var createCommand = new CreateShipmentCommand(new CreateShipmentDto
            {
                ...
                PickupLatitude = 29.9602,
                PickupLongitude = 31.2569,
                DeliveryAddress = "Nasr City, Cairo",
                DeliveryLatitude = 30.0626,
                DeliveryLongitude = 31.3468,
                Weight = 18m,
                DistanceKm = 14m,
                ShippingCost = 140m,
                Priority = ShipmentPriority.Normal,
            ...
            // 4. Trigger Hangfire Background Expiration Job
            using (var scope = _factory.Services.CreateScope())
            {
                var expirationService = scope.ServiceProvider.GetRequiredService<IAssignmentExpirationService>();
                await expirationService.ExpireAssignmentsAsync();
            ...
            // 5. Verify Assignment is Expired and Shipment remains unassigned Pending
            using (var scope = _factory.Services.CreateScope())
            {
                var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
                ...
                var dbDriver1 = await db.Drivers.FindAsync(driver1.Id);
                dbDriver1.Should().NotBeNull();
                dbDriver1!.Status.Should().Be(DriverStatus.Available);
            }

            // 6. Now Driver 2 is seeded and becomes available
            var (drv2User, driver2) = await TestAuthHelper.SeedDriverAsync(
                _factory.Services,
                email: $"driver2_exp_man_{Guid.NewGuid():N}@test.com",
                status: DriverStatus.Available);
            var drv2Token = await TestAuthHelper.GenerateJwtTokenAsync(_factory.Services, drv2User.Id, drv2User.Email!, drv2User.UserName!, Roles.Driver);
            var driver2Client = TestAuthHelper.CreateAuthenticatedClient(_factory, drv2Token);

            // 7. Dispatcher manually assigns Driver 2
            ...
            // 9. Verify shipment is now Assigned to Driver 2
            using (var scope = _factory.Services.CreateScope())
            {
                var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
